...
### F. Closed-Form Opportunity Audit (MANDATORY)

After all production runs complete, scan every figure- and table-generation
script under `pipeline/phase3_write/` (and reused validated code) for
stochastic computations whose target quantity has a known closed form. The
goal is to catch cases where simulation is used by inertia even though the
quantity is a normal/beta/gamma CDF (or finite combination of them) that
the project already computes analytically elsewhere.

**Why this matters.** A late-stage reviewer concern that recurs is:
"Why is this number computed by Monte Carlo when a closed form is
available?" Catching simulation-where-analytic-exists in Phase 3 saves
a revision round and prevents the manuscript from advertising MC SEs
that aren't real (the closed form is exact).

Write the canonical audit file to
`pipeline/phase3_write/briefings/closed_form_audit.md`:

```markdown
# Closed-Form Opportunity Audit (paper-modeler-owned)

## Scan targets
- `pipeline/phase3_write/` — every `.R`, `.py`, or notebook that produces a
  figure or table.
- `pipeline/phase2_validate/validated_code/` — for cross-referencing closed-form
  helpers (e.g., `implied_type1_error`, `power_at_dstar`, scipy.stats.*.cdf).

## Detection rule
For each file, find every occurrence of: `set.seed`, `rnorm`, `rbinom`,
`rgamma`, `runif`, `rt`, `np.random.*`, `rng.normal`, `rng.binomial`,
`rng.choice`, `rng.gamma`, or `MC` / `monte carlo` in comments. For each
hit, identify the *target quantity* (the value the script ultimately writes
to a CSV or figure).

## Closed-form availability check
| File:line | Stochastic call | Target quantity | Closed-form available in code? | Recommended helper | Action |
|-----------|-----------------|-----------------|--------------------------------|--------------------|--------|
| gen_X.R:42 | rnorm + posterior cutoff | Pr(reject \| δ*) | YES | `power_at_dstar()` (validated_code/method.R:87) | REPLACE |
| gen_Y.R:30 | rbinom + integration | Pr(success \| p) | NO (no analytic helper) | — | KEEP_MC |
| gen_Z.R:55 | rnorm + Gibbs step | posterior summary | UNKNOWN (intractable joint) | — | KEEP_MC |
| ... | ... | ... | ... | ... | ... |

## Provenance
- Validated code surveyed: pipeline/phase2_validate/validated_code/
  (commit / mtime: ...)
- Generated by: paper-modeler, [timestamp]

## Discussion contract
- paper-writer MUST NOT describe a quantity as "estimated via Monte Carlo
  with M = N replicates" if its row above is REPLACE. The script must be
  switched to the closed-form helper before the manuscript references the
  quantity.
- paper-fixer reads this file and applies REPLACE actions when fixing
  rigor or correctness issues.
- paper-grader's Code Auditor flags any REPLACE row left unaddressed
  (Rigor −0.5 per occurrence, capped at −1).
- consistency-auditor Pass 6 (figure consistency) cross-checks that captions
  do not advertise MC SEs for quantities flagged REPLACE.

## Action labels
- **REPLACE** — closed form exists in the project's own validated code; the
  simulation should be replaced. List the helper function and its file:line.
- **KEEP_MC** — no closed form exists, OR closed form exists in theory but
  requires intractable integration (high-dimensional, non-conjugate, etc.).
  One-line justification required.
- **NEEDS_REVIEW** — paper-modeler is uncertain; flag for human review
  (counts as a CRITICAL row).
```

**Heuristic rules of thumb** (apply during the scan, not as a substitute
for project-specific judgment):
- Conjugate Normal/Normal posterior tail probabilities → normal CDF.
- Beta-Binomial posterior tails → beta CDF (or `pbeta`).
- Gaussian power / type-I-error calculations under linear test statistics →
  shifted-normal CDFs.
- Range-calibration / sup-TIE optimisation → univariate root-find on a
  closed-form expression, NOT a simulation grid search.

**Failure mode to avoid**: don't recommend REPLACE for cases where the
simulation captures something the closed form misses (e.g., finite-sample
small-cell behaviour, asymmetric loss functions with no analytic form).
When in doubt, mark NEEDS_REVIEW.
...
- pipeline/phase3_write/briefings/closed_form_audit.md (canonical)
