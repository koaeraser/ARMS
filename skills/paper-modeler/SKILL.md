...
4. If MISMATCH: the CODE is authoritative (it produced the validated results).
   Compute the **code-correct LaTeX** by reading the code and writing the
   equation that the code actually implements.
...
## Verification table
| Eq# | Spec location (file:section) | LaTeX in spec | Code location (file:lineno) | Code expression | Status (MATCH | MISMATCH | NEW_IN_PAPER) | Code-correct LaTeX (if MISMATCH) |
|-----|------------------------------|---------------|-----------------------------|-----------------|---------------|--------------------------------|
...
For every MISMATCH row, the CODE expression is canonical. The manuscript MUST
display the code-correct LaTeX (paper-writer copies it verbatim). The spec is
updated post-hoc only via a Phase 1 RETHINK.

## Downstream consumers (do not duplicate this work)
- paper-writer: copy the "Code-correct LaTeX" column into manuscript.tex
- paper-grader: verify this file exists and has zero unresolved MISMATCH rows
- paper-fixer: when fixing a formula, READ this file (do not re-derive)
- paper-critic: when challenging a formula, cite the row number from this file
```
